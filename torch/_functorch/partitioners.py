    ...
    if inductor_config.pattern_matcher:
        for pass_name in inductor_config.post_grad_fusion_options:
            if pass_name == "activation_quantization_aten_pass":
                ...
                for fwd_node in fwd_module_outputs:
                    quant_fwd_output = fwd_node.meta.get("quant_node", None)
                    if quant_fwd_output:
                        quant_node = fwd_node.meta.pop("quant_node")
                        bwd_input = bwd_module_inputs[fwd_node.name]
                        with bwd_module.graph.inserting_after(bwd_input):
                            quant_bwd_input = bwd_module.graph.placeholder(
                                name=quant_node.name
                            )
                        dequant_type = bwd_input.meta["dequant_type"]
                        quant_bwd_input.meta.update(quant_fwd_output.meta)
                ...
                # update the bwd_inputs if quantization with scaling is used due to the
                if torch._inductor.config.post_grad_fusion_options[
                    "activation_quantization_aten_pass"
                ].get("use_scaling", True):
                    # update the corresponding bwd input nodes
                    # find the last non-tangent node
...
def quantize_activation_fw(match: Match, *args, **kwargs):
    ...
    for node in fwd_outputs:
        ...
        if node.meta.get("saved_for_quantization", False):
            ...
            node.meta["quant_node"] = quant_node
    ...
    # add the scale nodes to the ouput
    # find the first sym_node in the output
